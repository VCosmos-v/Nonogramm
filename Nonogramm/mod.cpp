...
void playGame(Grid& solution) {
    ...
    std::cout << "Команды:\n";
    std::cout << " fill x y   - закрасить клетку\n";
    std::cout << " clear x y  - очистить клетку\n";
    std::cout << " show       - показать поле\n";
    std::cout << " hints      - показать подсказки\n";
    std::cout << " exit       - выход\n";
    ...
    while (true) {
        ...
        if (cmd == "exit") break;
            ...
            if (checkSolution(player, rowHints, colHints)) {
                std::cout << "\nВы решили нонограмму!\n";
                break;
            }
        }

        else {
            std::cout << "Неизвестная команда. Используй fill/clear/show/hints/exit.\n";
        }
    }
}